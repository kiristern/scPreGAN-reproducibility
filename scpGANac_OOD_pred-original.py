...
def train_scPreGAN(config, opt):
    ...
    pred_perturbed_adata.write_h5ad(os.path.join(opt['outf'], 'pred_adata/', f'scpGANac_pred_{opt["prediction_type"]}.h5ad'))
    ...
    pred_z_adata.write_h5ad(
        os.path.join(opt['outf'], 'pred_z_adata', f'scpGANac_pred_z_{opt["prediction_type"]}.h5ad'))
...
def main(data_name):
    if data_name == 'pbmc':
        opt = {
            'cuda': True,
            'dataPath': './datasets/train_pbmc.h5ad',
            'checkpoint_dir': None,
            ...
            'data_name': 'pbmc',
            'model_name': 'pbmc_OOD',
            'outf': 'scpGANac_original/',
            'validation': False,
            'valid_dataPath': None,
            'use_sn': True,
            'use_wgan_div': True,
            'gan_loss': 'wgan',
            'train_flag': True, # change to True
            'n_classes': 7,
        }
    ...
    cell_type_list = ['CD14+Mono', 'B', 'Dendritic', 'NK']
